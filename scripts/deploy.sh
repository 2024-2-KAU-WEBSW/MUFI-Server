...
BUILD_PATH=$(ls /home/ubuntu/mufi-server/mufiServer-0.0.1-SNAPSHOT.jar)
JAR_NAME=$(basename $BUILD_PATH)
echo "[$NOW_TIME] build 파일명: $JAR_NAME"

echo "[$NOW_TIME] build 파일 복사"
DEPLOY_PATH=/home/ubuntu/mufi-server/nonstop/jar/
cp $BUILD_PATH $DEPLOY_PATH

echo "[$NOW_TIME] 현재 구동중인 Profile 확인"
CURRENT_PROFILE=$(curl -s https://www.mufi-server.o-r.kr/profile)
...
IDLE_APPLICATION=$IDLE_PROFILE-Mufi-Server.jar
...
nohup java -jar -Duser.timezone=Asia/Seoul -Dspring.profiles.active=$IDLE_PROFILE $IDLE_APPLICATION_PATH >> /home/ubuntu/mufi-server/deploy.log 2>/home/ubuntu/mufi-server/deploy_err.log &
